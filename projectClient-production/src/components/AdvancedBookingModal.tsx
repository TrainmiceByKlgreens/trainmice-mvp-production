...
export function AdvancedBookingModal({
  ...
  const handleSubmitPublicBooking = async () => {
    ...
    if (!formData.companyName || !formData.address || !formData.userName || !formData.userEmail || !formData.courseMode) {
    ...
    try {
      ...
      alert(
        `Your public session request has been sent to the Admin team. They will review and get back to you soon.`
      );

      setTimeout(() => {
        onClose();
        setFormData({ companyName: '', address: '', userName: '', userEmail: '', courseMode: '' });
        setSelectedMonth('');
      }, 2000);
    } catch (error) {
