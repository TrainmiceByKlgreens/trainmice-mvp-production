...
export class ApiClient {
  ...
  async updateClientProfile(data: {
    ...
    return this.put<{ client: any; message: string }>('/auth/client/profile', data);
  }

  /**
   * Get current client's public event registrations
   */
  async getMyRegistrations(): Promise<any[]> {
    try {
      const response = await this.get<{ registrations: any[] }>('/events/my-registrations');
  ...
  /**
   * Get current client's in-house booking requests
   */
  async getMyBookingRequests(): Promise<any[]> {
    try {
      const response = await this.get<{ bookings: any[] }>('/bookings/my-bookings');
  ...
  private normalizeTrainerLanguages(raw: any): string[] {
    if (Array.isArray(raw.trainerLanguages) && raw.trainerLanguages.length > 0) {
      return raw.trainerLanguages
        .map((language: any) => language?.language || language?.name || null)
        .filter((language: string | null): language is string => Boolean(language));
    }

    return this.parseStringArray(raw.languagesSpoken ?? raw.languages_spoken ?? raw.languages);
  }

  private normalizeCourseSchedule(rawSchedule: unknown): any[] {
    ...
    return rawSchedule.map((item: any) => ({
      ...
      module_title: item.moduleTitle ?? item.module_title ?? '',
      submoduleTitle: item.submoduleTitle ?? item.submodule_title ?? [],
      submodule_title: item.submoduleTitle ?? item.submodule_title ?? [],
      durationMinutes: item.durationMinutes ?? item.duration_minutes ?? null,
      duration_minutes: item.durationMinutes ?? item.duration_minutes ?? null,
    }));
  }
  ...
  private mapCourse(raw: any): any {
    ...
    const normalizedSchedule = this.normalizeCourseSchedule(raw.courseSchedule ?? raw.course_schedule ?? []);
    ...
    return {
      ...
      brochure_url: this.resolveImageUrl(raw.brochureUrl ?? raw.brochure_url ?? null),
      image_url: this.resolveImageUrl(raw.imageUrl ?? raw.image_url ?? null),
      ...
      delivery_languages: raw.deliveryLanguages ?? raw.delivery_languages ?? [],
      courseSchedule: normalizedSchedule,
      course_schedule: normalizedSchedule,
      course_trainers: (raw.courseTrainers ?? raw.course_trainers ?? []).map((ct: any) => ({
        ...ct,
        trainer: this.mapTrainer(ct.trainer)
      })),
    };
  }
  ...
  private mapTrainer(raw: any): any {
    ...
    const topics = this.parseStringArray(raw.areasOfExpertise ?? raw.areas_of_expertise ?? raw.topics);
    const languages = this.normalizeTrainerLanguages(raw);
    ...
    return {
      ...
      profile_pic: this.resolveImageUrl(raw.profilePic ?? raw.profile_pic ?? null),
      ...
      workHistoryEntries: raw.workHistoryEntries ?? raw.workHistory ?? [],
      pastClients: raw.pastClients ?? [],
      trainerLanguages: raw.trainerLanguages ?? [],
      languages,
    };
  }
