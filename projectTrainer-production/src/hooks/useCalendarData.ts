    ...
    try {
      ...
      const filteredBookings: BookingWithCourse[] = allBookings
        ...
        .filter((b) => {
          ...
          // Filter out cancelled or denied bookings to prevent "ghost" entries
      ...
      const eventBookings: BookingWithCourse[] = allEvents
        .map((event: any) => {
          ...
          const coursesObject = {
            ...
            image_url: event.imageUrl || courseData.imageUrl || null,
      ...
      // Fetch manual bookings from TrainerBooking table
      const trainerBookings = await apiClient.getTrainerBookings(trainerId, {
        startDate: startStr,
        endDate: endStr,
      });
