...
export interface TrainerLanguage {
  ...
  trainer_id: string;
  language: string;
  proficiency: 'Native' | 'Professional' | 'Intermediate' | 'Beginner';
  created_at: string;
}
...
export interface Course {
  ...
  status: 'DRAFT' | 'PENDING_APPROVAL' | 'APPROVED' | 'DENIED' | 'draft' | 'published';
  isRead?: boolean;
  course_sequence: number | null;
  delivery_languages?: string[] | null;
  created_at: string;
  course_trainers?: CourseTrainer[];
  ...
  courseNotes?: {
    ...
    createdAt: string;
    created_at?: string;
    trainer?: { fullName: string; profilePic?: string | null };
  }[];
}
