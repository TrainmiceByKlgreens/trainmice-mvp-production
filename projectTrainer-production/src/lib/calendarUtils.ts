...
export function getBookingsForDate(
  ...
  return bookings.filter((booking) => {
    // [NEW] Precise Matching: Handle both availability IDs (legacy) and date strings (new)
    if (booking.selected_dates && Array.isArray(booking.selected_dates) && booking.selected_dates.length > 0) {
      const matchedById = availability ? booking.selected_dates.includes(availability.id) : false;
      const matchedByDate = booking.selected_dates.includes(dateString);
      return matchedById || matchedByDate;
    }
...
export function resolveDayStatus(
  ...
  // CONSIDER AS TENTATIVE: approved, tentative, quoted
  const tentativeBookings = bookings.filter((b) => {
    const status = toLowerStatus(b.status);
    return status === 'approved' || status === 'tentative' || status === 'quoted';
  });
  if (tentativeBookings.length > 0) return 'tentative';

  // If no bookings, fallback to trainer_availability status
  if (availability) {
...
export function buildCalendarDays(
  ...
  return dates.map((date) => {
    ...
    const dayBookings = getBookingsForDate(bookings, date, availability);
    const isBlocked = isDateBlocked(date, blockedWeekdays);
    
    // Find manual booking for this date
    const manualBooking = manualBookings.find((b: any) => {
      const bDate = b.bookingDate || b.booking_date;
      return bDate && bDate.split('T')[0] === dateString;
    });
    ...
    return {
      ...
      availability: manualBooking ? { ...availability, notes: manualBooking.notes, status: (manualBooking.status || '').toLowerCase() } : availability,
      isBlocked
    } as CalendarDay;
  });
}
