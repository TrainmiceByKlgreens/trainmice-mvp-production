...
export class ApiClient {
  ...
  async uploadTrainerProfileImage(id: string, file: File) {
    const formData = new FormData();
    formData.append('image', file);
    const response = await this.post<{ trainer: any }>(`/trainers/${id}/profile-image`, formData);
    return this.mapTrainer(response.trainer);
  }
  ...
  async markBookingRequestAsRead(id: string) {
    return this.put<{ bookingRequest: any }>(`/bookings/${id}/read`);
  }

  // Trainer Bookings (Manual/Self-Bookings)
  ...
  async updateBookingStatus(id: string, status: string, availabilityIds?: string[]) {
    const response = await this.put<{ bookingRequest: any }>(`/bookings/${id}/status`, { status, availabilityIds });
    return response.bookingRequest;
  }
  ...
  async sendMessageToAdmin(data: {
    ...
    if (data.attachment) {
      ...
      return this.post<{ thread: any; message: any }>('/trainer/messages', formData);
    }
  ...
  async getUnreadEventIds() {
    return this.get<{ eventIds: string[]; enquiryIds: string[] }>('/event-enquiry-messages/trainer/unread-ids');
  }
  ...
  async replyToEventEnquiry(enquiryId: string, message: string, attachment?: File) {
    if (attachment) {
      ...
      return this.post<{ newMessage: any }>(`/event-enquiry-messages/trainer/${enquiryId}/reply`, formData);
    }
