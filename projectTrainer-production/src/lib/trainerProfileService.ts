...
export async function fetchTrainerLanguages(trainerId: string): Promise<TrainerLanguage[]> {
  try {
    const response = await apiClient.get<{ languages: any[] }>(`/trainers/${trainerId}/languages`);
    const langs = response.languages || [];

    // Map backend camelCase to frontend snake_case
    return langs.map((l: any) => ({
      id: l.id,
      trainer_id: l.trainerId || l.trainer_id,
      language: l.language,
...
export async function createTrainerLanguage(
  languageData: Omit<TrainerLanguage, 'id' | 'created_at'>
): Promise<TrainerLanguage> {
  try {
    const response = await apiClient.post<{ language: any }>(
      `/trainers/${languageData.trainer_id}/languages`,
      {
        language: languageData.language,
        proficiency: languageData.proficiency
      }
    );
    const l = response.language;
...
export async function updateTrainerLanguage(
  trainerId: string,
  id: string,
  proficiency: TrainerLanguage['proficiency']
): Promise<TrainerLanguage> {
  try {
    const response = await apiClient.put<{ language: any }>(
      `/trainers/${trainerId}/languages/${id}`,
      { proficiency }
    );
    const l = response.language;
...
export async function deleteTrainerLanguage(trainerId: string, id: string): Promise<void> {
  try {
    await apiClient.delete(`/trainers/${trainerId}/languages/${id}`);
  } catch (error: any) {
