...
import { Plus, Edit, Trash2, Calendar, MapPin, CheckCircle, XCircle, File, Clock, Star, X, Download, Send, ChevronDown, ChevronUp, ArrowLeft, Copy, MessageSquare } from 'lucide-react';
...
export const EnhancedCoursesPage: React.FC = () => {
  ...
  const [newNoteType, setNewNoteType] = useState<'INTERNAL' | 'TO_TRAINER'>('INTERNAL');
  ...
  useRealtime((payload) => {
    if (['courses', 'course_notes'].includes(payload.table)) {
      console.log('📚 Courses/Notes updated, refreshing data...');
      fetchData();
    }
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const mappedCourses: Course[] = coursesData.map((c: any) => ({
        ...
        brochureUrl: c.brochureUrl || c.brochure_url || null,
  ...
  const handleViewNotes = async (course: Course) => {
    // Optimistic update for the red dot
    setCourses(prev => prev.map(c => {
      if (c.id === course.id && c.courseNotes) {
        return {
          ...c,
          courseNotes: c.courseNotes.map((n: any) => ({ ...n, isRead: true }))
        };
      }
      return c;
    }));
    ...
    try {
      ...
      setSelectedCourseForNotes(courseData.course as any);
  ...
  const handleAddNote = async () => {
    if (!selectedCourseForNotes || !newNote.trim()) return;
    try {
      setSubmittingNote(true);
      await apiClient.addAdminCourseNote(selectedCourseForNotes.id, {
        note: newNote,
        type: newNoteType,
      });
      // Refresh the notes
      const courseData = await apiClient.getAdminCourse(selectedCourseForNotes.id);
      setSelectedCourseForNotes(courseData.course as any);
      setNewNote('');
      showToast('Note added successfully', 'success');
    } catch (error: any) {
      showToast(error.message || 'Error adding note', 'error');
    } finally {
        ...
        <Card>
          <div className="p-6">
            <CourseForm
              ...
              onSubmit={async (data, trainerIds, imageFile) => {
                try {
                  let courseId = '';
                  if (mode === 'create') {
                    const response = await apiClient.createAdminCourse({
                      ...data,
                      trainerIds: trainerIds // Backward compatibility
                    });
                    courseId = response.course.id;
                    showToast('Course added successfully', 'success');
                  } else if (mode === 'edit' && editingCourse) {
                    const response = await apiClient.updateAdminCourse(editingCourse.id, {
                      ...data,
                      trainerIds: trainerIds // Backward compatibility
                    });
                    courseId = response.course.id;
                  ...
                  if (imageFile && courseId) {
                    try {
                      await apiClient.uploadCourseImage(courseId, imageFile);
                      showToast('Course image uploaded successfully', 'success');
                    } catch (uploadError: any) {
                      showToast(`Course saved, but image upload failed: ${uploadError.message}`, 'warning');
                    }
                  }
                  ...
                  // Refresh courses
                  const coursesData = await apiClient.getAdminCourses();
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="p-6">
                      ...
                      <div className="flex-1 relative">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {course.status === 'PENDING_APPROVAL' && (
                            <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse flex-shrink-0" title="Approval required" />
                          )}
                        </div>
                    ...
                    {course.courseNotes && course.courseNotes.length > 0 && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                        <div className="flex items-center text-yellow-800 font-semibold mb-2 text-sm">
                          <MessageSquare size={14} className="mr-1.5" />
                          Notes from Trainer ({course.courseNotes.length})
                        </div>
                        <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar pr-2 text-xs text-gray-700">
                          {course.courseNotes.map((note: any) => (
                            <div key={note.id} className="border-b border-yellow-100 last:border-0 pb-2 last:pb-0">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-600 mb-4">
                     ...
                     <div className="flex space-x-2">
                      ...
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => handleViewNotes(course)}
                        className="relative"
                        title="Course Notes"
                      >
                        <MessageSquare size={16} />
                        {course.courseNotes?.some((n: any) => n.isRead === false) && (
      ...
      {/* Admin Created & Approved Courses */}
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          All Courses ({filteredCourses.filter(c => c.status !== 'PENDING_APPROVAL').length})
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses
      ...
      <Modal
        ...
        title={`Course Notes - ${selectedCourseForNotes?.title || ''}`}
        size="lg"
      >
        {selectedCourseForNotes && (
          <div className="space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {(!selectedCourseForNotes?.courseNotes || selectedCourseForNotes.courseNotes.length === 0) ? (
                  ...
                  <div className="space-y-2">
                    {selectedCourseForNotes.courseNotes.map((note: any) => (
              ...
              {/* Notes To Trainer */}
              <div className="pt-2">
                <h3 className="font-semibold text-teal-800 mb-2 border-b pb-1">Notes To Trainer</h3>
                {(() => {
                  const toTrainerNotes = (selectedCourseForNotes as any).adminCourseNotes?.filter((n: any) => n.type === 'TO_TRAINER') || [];
                  if (toTrainerNotes.length === 0) {
                    return <p className="text-sm text-gray-500 italic">No notes sent to trainer.</p>;
                  }
                  return (
                    <div className="space-y-2">
                      {toTrainerNotes.map((note: any) => (
                        <div key={note.id} className="bg-teal-50 p-3 rounded-md text-sm border border-teal-100">
                          <p className="text-teal-900">{note.note}</p>
                          <p className="text-xs text-teal-700 mt-2 text-right">
                            {note.admin?.fullName || 'Admin'} - {new Date(note.createdAt || note.created_at).toLocaleString()}
                          </p>
                        </div>
                      ))}
                    </div>
              ...
              {/* Internal Notes */}
              <div className="pt-2">
                <h3 className="font-semibold text-indigo-800 mb-2 border-b pb-1">Admin Internal Notes</h3>
                {(() => {
                  const internalNotes = (selectedCourseForNotes as any).adminCourseNotes?.filter((n: any) => n.type === 'INTERNAL') || [];
                  if (internalNotes.length === 0) {
                    return <p className="text-sm text-gray-500 italic">No internal admin notes.</p>;
                  }
                  return (
                    <div className="space-y-2">
                      {internalNotes.map((note: any) => (
                        <div key={note.id} className="bg-indigo-50 p-3 rounded-md text-sm border border-indigo-100">
                          <p className="text-indigo-900">{note.note}</p>
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-6">
              <h4 className="font-medium text-gray-800 mb-3">Add New Note</h4>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Note Type</label>
                  <Select
                    value={newNoteType}
                    onChange={(e) => setNewNoteType(e.target.value as 'INTERNAL' | 'TO_TRAINER')}
                    options={[
                      { value: 'INTERNAL', label: 'Internal Note (Admins Only)' },
                      { value: 'TO_TRAINER', label: 'Message to Trainer' },
                    ]}
                  />
                </div>
                <Textarea
