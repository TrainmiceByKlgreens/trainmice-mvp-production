                                ...
                                <div className="overflow-hidden bg-white rounded-lg border border-gray-200">
                                    <ul className="divide-y divide-gray-200 overflow-y-auto max-h-[600px]">
                                        {event.registrations.map((reg) => (
                                            <li
                                                key={reg.id}
                                                className={`p-4 hover:bg-gray-50 transition-colors ${!reg.isRead ? 'border-l-4 border-l-red-500 bg-red-50/10' : ''}`}
                                                onMouseEnter={() => {
                                                    if (!reg.isRead) onMarkRegistrationAsRead(reg.id);
                                                }}
                                            >
                                                <div className="flex items-start justify-between">
                                                    <div className="space-y-1.5 flex-1 cursor-default">
                                                        <div className="flex items-center">
                                                            ...
                                                            {!reg.isRead && (
                                                                <span className="ml-2 h-2 w-2 rounded-full bg-red-500 animate-pulse" title="New registration" />
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="flex flex-col items-end space-y-2 ml-4">
                                                        <div className="flex items-center gap-2">
                                                            {reg.status === 'REGISTERED' && (
                                                                <div className="flex gap-1">
                                                                    <Button
                                                                        variant="success"
                                                                        size="sm"
                                                                        className="h-7 w-7 p-0 flex items-center justify-center rounded-full bg-green-600 hover:bg-green-700 text-white shadow-sm"
                                                                    ...
                                                                    <Button
                                                                        variant="danger"
                                                                        size="sm"
                                                                        className="h-7 w-7 p-0 flex items-center justify-center rounded-full bg-red-600 hover:bg-red-700 text-white shadow-sm"
                                                            ...
                                                            {reg.status === 'APPROVED' && (
                                                                <Button
                                                                    variant="secondary"
                                                                    size="sm"
                                                                    className="h-7 w-7 p-0 flex items-center justify-center rounded-full border-red-200 text-red-600 bg-red-50 hover:bg-red-100"
                                                                    onClick={() => onCancelRegistration(reg.id)}
                                                                    disabled={isUpdating}
                                                        ...
                                                        <p className="text-xs text-gray-500 font-medium whitespace-nowrap">
                                                            {reg.numberOfParticipants || 1} Participant{((reg.numberOfParticipants || 1) > 1) ? 's' : ''}
                                                        </p>
                                                    </div>
                                                </div>
                                            </li>
