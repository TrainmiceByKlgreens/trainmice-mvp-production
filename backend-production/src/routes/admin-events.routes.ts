...
router.put('/registrations/:id/read', async (req: AuthRequest, res: Response) => {
  try {
    ...
    broadcastUpdate('event_registrations', 'UPDATE', { id: registration.id });

    return res.json({ registration, message: 'Registration marked as read' });
  } catch (error: any) {
    console.error('Mark registration as read error:', error);
    return res.status(500).json({ error: 'Failed to mark registration as read', details: error.message });
  }
});
    ...
    try {
      ...
      if (status === 'CANCELLED' && event.trainerId) {
        try {
          ...
          // [NEW] Sync BookingRequest status to CANCELLED
          // Match by trainer, course, and date (synced in confirm route)
          try {
            const linkedBooking = await prisma.bookingRequest.findFirst({
              where: {
                trainerId: event.trainerId,
                courseId: event.courseId,
                requestedDate: event.eventDate,
                status: 'CONFIRMED',
              },
            });
            ...
            if (linkedBooking) {
              ...
              console.log(`[Event Cancellation Sync] Marked linked booking ${linkedBooking.id} as CANCELLED`);
            }
          } catch (syncError) {
            console.error('Error syncing booking status during event cancellation:', syncError);
          }
