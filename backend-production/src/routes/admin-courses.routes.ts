...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updateResult = await prisma.courseNote.updateMany({
      where: {
    ...
    if (updateResult.count > 0) {
      broadcastUpdate('course_notes', 'UPDATE', {});
    }
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const allowedFields = [
        ...
        'professionalDevelopmentPoints', 'professionalDevelopmentPointsOther', 'imageUrl', 'deliveryLanguages',
      ];

      // Map image_url from frontend to imageUrl for Prisma
      ...
      allowedFields.forEach(field => {
        if (req.body[field] !== undefined) {
          ...
          } else if (['learningObjectives', 'learningOutcomes', 'targetAudience', 'methodology', 'prerequisite', 'modules', 'deliveryLanguages'].includes(field)) {
            // Handle JSON arrays
      ...
      const { trainerAvailabilityId } = req.body;
      ...
      // Actually create the course here
      const course = await prisma.course.create({
        data: updateData,
      });

      // Extract trainer role fields
      const { primaryTrainerId, coTrainerIds } = req.body;

      // Handle trainer assignments for admin-created courses
      if (primaryTrainerId || (coTrainerIds && coTrainerIds.length > 0)) {
        const trainersToCreate = [];
        if (primaryTrainerId) {
          trainersToCreate.push({
            courseId: course.id,
            trainerId: primaryTrainerId,
            role: 'PRIMARY' as any,
          });
          // Update the main trainerId field as well
          await prisma.course.update({
            where: { id: course.id },
            data: { trainerId: primaryTrainerId },
          });
        }
        if (coTrainerIds && Array.isArray(coTrainerIds)) {
          coTrainerIds.forEach((id: string) => {
            if (id !== primaryTrainerId) {
              trainersToCreate.push({
                courseId: course.id,
                trainerId: id,
                role: 'CO_TRAINER' as any,
              });
            }
          });
        }

        if (trainersToCreate.length > 0) {
          await prisma.courseTrainer.createMany({
            data: trainersToCreate,
          });
        }
      } else if (req.body.trainerIds && Array.isArray(req.body.trainerIds) && req.body.trainerIds.length > 0) {
        // Legacy support
        const trainerIds = req.body.trainerIds;
        await prisma.courseTrainer.createMany({
          data: trainerIds.map((id: string, index: number) => ({
            courseId: course.id,
            trainerId: id,
            role: index === 0 ? 'PRIMARY' : 'CO_TRAINER',
          })),
        });
        await prisma.course.update({
          where: { id: course.id },
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const existingCourse = await prisma.course.findUnique({
        where: { id: req.params.id },
        select: { fixedDate: true, trainerId: true } as any,
      });
      ...
      // Sync title with associated events if it was updated
      if (updateData.title) {
        try {
          const syncResult = await prisma.event.updateMany({
            where: { courseId: course.id },
            data: { title: updateData.title },
          });
          console.log(`Synced title for ${syncResult.count} associated event(s)`);
        } catch (syncError) {
          console.error('Error syncing event titles:', syncError);
          // Don't fail the course update if sync fails, but log it
        }
      }
      ...
      if ((oldFixedDate as any) !== (newFixedDate as any) && course.trainerId) {
        // If old fixedDate existed, mark that availability as AVAILABLE again
        if (oldFixedDate) {
          try {
            const oldDate = new Date(oldFixedDate as any);
      ...
      // Extract trainer role fields
      const { primaryTrainerId, coTrainerIds, trainerIds: legacyTrainerIds } = req.body;

      // Handle trainer assignments with roles
      if (primaryTrainerId !== undefined || coTrainerIds !== undefined) {
        ...
        if (primaryTrainerId) {
          ...
          // Update the main trainerId field
          await prisma.course.update({
            where: { id: course.id },
            data: { trainerId: primaryTrainerId },
          });
        } else if (primaryTrainerId === null) {
          // Clear the main trainerId field
          await prisma.course.update({
            where: { id: course.id },
            data: { trainerId: null },
          });
      ...
      } else if (legacyTrainerIds && Array.isArray(legacyTrainerIds)) {
        // Legacy support
        await prisma.courseTrainer.deleteMany({
          where: { courseId: course.id },
        });
        if (legacyTrainerIds.length > 0) {
          await prisma.courseTrainer.createMany({
            data: legacyTrainerIds.map((id: string, index: number) => ({
              courseId: course.id,
              trainerId: id,
              role: index === 0 ? 'PRIMARY' : 'CO_TRAINER',
            })),
          ...
          await prisma.course.update({
            where: { id: course.id },
            data: { trainerId: legacyTrainerIds[0] },
          });
        } else {
          await prisma.course.update({
            where: { id: course.id },
...
router.put('/:id/approve', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await prisma.course.update({
      ...
      data: { 
        status: 'APPROVED',
        isRead: false // Trigger red dot for trainer
      },
      include: {
        trainer: {
          select: {
    ...
    broadcastUpdate('courses', 'UPDATE', { courseId: course.id });
...
router.put('/:id/reject', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (rejectionReason) {
      await prisma.adminCourseNote.create({
        data: {
          courseId: course.id,
          adminId: req.user!.id,
    ...
    broadcastUpdate('courses', 'UPDATE', { courseId: course.id });
    if (rejectionReason) {
      broadcastUpdate('admin_course_notes', 'CREATE', { courseId: course.id });
    }
...
// Add course admin note
router.post(
  '/:id/admin-notes',
  [
    body('note').notEmpty().trim(),
    body('type').isIn(['INTERNAL', 'TO_TRAINER']),
  ],
  async (req: AuthRequest, res: Response) => {
    try {
      const errors = validationResult(req);
      ...
      const adminNote = await prisma.adminCourseNote.create({
        data: {
          courseId: req.params.id,
          adminId: req.user!.id,
          note: req.body.note,
      ...
      await createActivityLog({
        userId: req.user!.id,
        actionType: 'CREATE',
        entityType: 'admin_course_note',
        entityId: adminNote.id,
        description: `Added ${req.body.type} note to course: ${course.title}`,
      });

      // Fetch admin info so frontend can display it
      const createdNote = await prisma.adminCourseNote.findUnique({
        where: { id: adminNote.id },
      });

      broadcastUpdate('admin_course_notes', 'CREATE', { courseId: req.params.id });

      return res.status(201).json({ adminNote: createdNote });
    } catch (error: any) {
      console.error('Add admin note error:', error);
      return res.status(500).json({ error: 'Failed to add admin note', details: error.message });
    }
  }
);
  ...
  try {
    ...
    const courseTrainer = await prisma.courseTrainer.create({
      data: {
        courseId: req.params.id,
        trainerId: trainerId,
        role: (role as any) || 'CO_TRAINER', // Use role from body or default to CO_TRAINER
      } as any,
      include: {
        trainer: {
          select: {
    ...
    // Update course's trainer_id if it's PRIMARY or not set yet
    if (role === 'PRIMARY' || !hasPrimaryTrainer) {
      await prisma.course.update({
        where: { id: req.params.id },
...
router.put('/:id/assign-trainer', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Also update/create CourseTrainer relationship
    ...
    // Set other trainers to CO_TRAINER for consistency if needed, 
    // but typically assign-trainer implies setting the main one.
    ...
    try {
      ...
      const event = await prisma.event.create({
        data: {
          ...
          maxPacks: maxPacks ? parseInt(maxPacks) : null,
          professionalDevelopmentPoints: professionalDevelopmentPoints !== undefined
            ? professionalDevelopmentPoints
            : course.professionalDevelopmentPoints,
          professionalDevelopmentPointsOther: professionalDevelopmentPointsOther !== undefined
            ? professionalDevelopmentPointsOther
            : course.professionalDevelopmentPointsOther,
        },
        include: {
          trainer: {
            select: {
