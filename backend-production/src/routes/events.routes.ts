  ...
  try {
    ...
    const eventsWithParticipants = events.map(event => {
      ...
      return {
        ...
        selected_dates: (event as any).selectedDates,
      };
    });
...
// Get current client's own event registrations (dedicated endpoint for My Bookings page)
// Only returns PUBLIC-type events — IN_HOUSE events are shown via /bookings/my-bookings
  ...
  try {
    ...
    const registrations = await prisma.eventRegistration.findMany({
      where: {
        ...
        // Exclude registrations linked to IN_HOUSE events.
        // When an admin confirms an in-house booking they create an Event with courseType=['IN_HOUSE']
        // and an EventRegistration for the client. We must exclude those here so they don't appear
        // in the Public tab — the in-house journey is handled by /bookings/my-bookings.
        event: {
          NOT: {
            courseType: {
              // Prisma JSON filter: exclude arrays containing 'IN_HOUSE'
              array_contains: 'IN_HOUSE',
            },
          },
        },
      },
      include: {
        event: {
          select: {
            ...
            state: true,
            category: true,
            status: true,      // needed for combined status logic
            courseType: true,  // returned so frontend can double-check
            course: {
              select: {
                id: true,
                title: true,
    ...
    const mapped = registrations.map((r) => ({
      id: r.id,
      eventId: r.eventId,
      courseId: r.event?.courseId || null,
      courseTitle: r.event?.course?.title || r.event?.title || 'Event',
      courseCode: r.event?.course?.courseCode || null,
      eventDate: toDateStr(r.event?.eventDate) || '',
      startDate: toDateStr(r.event?.startDate),
      endDate: toDateStr(r.event?.endDate),
      venue: r.event?.venue || null,
      city: r.event?.city || null,
      state: r.event?.state || null,
      category: r.event?.category || null,
      // Both registration-level and event-level status — frontend combines them
      registrationStatus: r.status,
      eventStatus: r.event?.status || 'ACTIVE',
      courseType: r.event?.courseType || null,
      registeredAt: r.createdAt,
    }));

    return res.json({ registrations: mapped });
  } catch (error: any) {
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      // If event status is changed to CANCELLED, update trainer availability safely
      if (status === 'CANCELLED' && event.trainerId) {
        try {
          // Prepare dates for release: use selectedDates if available, fallback to eventDate
          const datesToRelease: (string | Date)[] = (event.selectedDates as string[]) || (event.eventDate ? [event.eventDate] : []);
          
          if (datesToRelease.length > 0) {
            await releaseTrainerAvailability(event.trainerId, event.id, datesToRelease);
          }
