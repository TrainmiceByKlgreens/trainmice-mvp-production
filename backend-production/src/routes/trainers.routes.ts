...
// Upload trainer profile image (trainer or admin)
router.post(
  '/:id/profile-image',
  authenticate,
  authorize('TRAINER', 'ADMIN'),
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (req.user!.role !== 'ADMIN' && req.user!.id !== trainerId) {
        return res.status(403).json({ error: 'Not authorized to update this profile image' });
      }
      ...
      await createActivityLog({
        ...
        entityType: 'trainer',
        entityId: trainerId,
        description: `Updated trainer profile image for ${trainer.fullName}`,
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });
      ...
      console.error('Upload trainer profile image error:', error);
      return res.status(500).json({ error: 'Failed to upload trainer profile image', details: error.message });
    }
  }
);
...
router.get('/:id/languages', authenticateOptional, async (req: AuthRequest, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch languages', details: error.message });
  }
});

// Add language (trainer only)
router.post(
  '/:id/languages',
  authenticate,
  authorize('TRAINER', 'ADMIN'),
  async (req: AuthRequest, res) => {
    try {
      ...
      const trainerLanguage = await prisma.trainerLanguage.create({
        data: {
          trainerId,
          language,
          proficiency,
      ...
      await createActivityLog({
        userId: trainerId,
        actionType: 'CREATE',
        entityType: 'trainer_language',
        entityId: trainerLanguage.id,
        description: `Trainer added language: ${language} (${proficiency})`,
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });

      return res.status(201).json({ language: trainerLanguage });
    } catch (error: any) {
      console.error('Create language error:', error);
      return res.status(500).json({ error: 'Failed to add language', details: error.message });
    }
  }
);

// Update language proficiency (trainer only)
router.put(
  '/:id/languages/:langId',
  authenticate,
  authorize('TRAINER', 'ADMIN'),
  async (req: AuthRequest, res) => {
    try {
      ...
      const existing = await prisma.trainerLanguage.findUnique({
        where: { id: langId },
      });
      ...
      const trainerLanguage = await prisma.trainerLanguage.update({
        where: { id: langId },
        data: {
      ...
      await createActivityLog({
        ...
        entityType: 'trainer_language',
        entityId: trainerLanguage.id,
        description: `Trainer updated language proficiency for ${trainerLanguage.language} to ${trainerLanguage.proficiency}`,
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });

      return res.json({ language: trainerLanguage });
    } catch (error: any) {
      console.error('Update language error:', error);
      return res.status(500).json({ error: 'Failed to update language', details: error.message });
    }
  }
);

// Delete language (trainer only)
router.delete(
  '/:id/languages/:langId',
  authenticate,
  authorize('TRAINER', 'ADMIN'),
  async (req: AuthRequest, res) => {
    try {
      ...
      await createActivityLog({
        ...
        entityType: 'trainer_language',
        entityId: langId,
        description: `Trainer removed language: ${existing.language}`,
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });
      ...
      return res.status(500).json({ error: 'Failed to remove language', details: error.message });
    }
  }
);
