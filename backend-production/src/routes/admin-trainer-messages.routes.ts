...
router.get('/trainer-messages/:trainerId/thread', async (req: AuthRequest, res: Response) => {
  try {
      ...
      include: {
        trainer: {
          select: {
            ...
            fullName: true,
            email: true,
            // fcmToken removed as it does not exist on Trainer model
          },
        },
        messages: {
    ...
    // If no thread exists, return empty structure with trainer specific info
    if (!thread) {
    ...
    const trainerMsgResult = await prisma.trainerMessage.updateMany({
      where: { trainerId: thread.trainerId, isRead: false },
      data: { isRead: true },
    });

    // Update thread and broadcast ONLY if changes were made
    if (updateResult.count > 0 || trainerMsgResult.count > 0 || thread.unreadCount > 0) {
      broadcastUpdate('messages', 'UPDATE', { trainerId });
      broadcastUpdate('message_threads', 'UPDATE', { trainerId });
      broadcastUpdate('trainer_messages', 'UPDATE', { trainerId });
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      broadcastUpdate('messages', 'CREATE', { trainerId });
      broadcastUpdate('message_threads', 'UPDATE', { trainerId });
      broadcastUpdate('trainer_messages', 'UPDATE', { trainerId });
...
router.put('/trainer-messages/:id/read', async (req: AuthRequest, res: Response) => {
  try {
    ...
    broadcastUpdate('trainer_messages', 'UPDATE', { id: req.params.id });
    broadcastUpdate('messages', 'UPDATE', { trainerId: message.trainerId });
    broadcastUpdate('message_threads', 'UPDATE', { trainerId: message.trainerId });
...
router.put('/event-enquiries/:id/read', async (req: AuthRequest, res: Response) => {
  try {
    ...
    broadcastUpdate('event_enquiries', 'UPDATE', { id: req.params.id });
