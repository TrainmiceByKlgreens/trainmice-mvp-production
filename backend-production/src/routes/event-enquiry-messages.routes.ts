...
// Get eventIds of enquiries with unread messages for trainer
router.get('/trainer/unread-ids', authorize('TRAINER'), async (req: AuthRequest, res: Response) => {
  try {
    const trainerId = req.user!.id;

    const unreadEnquiries = await prisma.eventEnquiry.findMany({
      where: {
        trainerId,
        unreadCount: { gt: 0 },
      },
    ...
    return res.json({
      eventIds: unreadEnquiries.map(e => e.eventId),
      enquiryIds: unreadEnquiries.map(e => e.id),
    });
  } catch (error: any) {
    console.error('Get unread ids error:', error);
    return res.status(500).json({ error: 'Failed to fetch unread ids', details: error.message });
  }
});
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      broadcastUpdate('event_enquiries', 'UPDATE', { id: enquiry.id, trainerId: enquiry.trainerId });
